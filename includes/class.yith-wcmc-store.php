...
if ( ! class_exists( 'YITH_WCMC_Store' ) ) {
	...
	class YITH_WCMC_Store {
		...
		public function execute_store_creation() {
			...
			if( $id ){
			    wp_redirect( add_query_arg( array( 'page' => 'yith_wcmc_panel', 'tab' => 'store' ), admin_url( 'admin.php' ) ) );
            }
		}
		...
		protected function _create_store( $list_id, $args = array() ){
			...
			$defaults = array(
				...
				'platform'      => 'woocommerce', //sprintf( '%s %s', __( 'YITH WooCommerce MailChimp', 'yith-woocommerce-mailchimp' ), YITH_WCMC_VERSION ),
		...
		public function get_sync_status_message() {
			...
			$status_message = sprintf(
				__( 'Current synchronization status: <b>Products:</b> %s/%s (%s%%) | <b>Coupons:</b> %s/%s (%s%%) | <b>Orders:</b> %s/%s (%s%%)', 'yith-woocommerce-mailchimp' ),
				$sync_info['product']['processed'],
				$sync_info['product']['count'],
				$sync_info['product']['count'] ? floor( $sync_info['product']['processed'] / $sync_info['product']['count'] * 100 ) : 100,
				$sync_info['shop_coupon']['processed'],
				$sync_info['shop_coupon']['count'],
				$sync_info['shop_coupon']['count'] ? floor( $sync_info['shop_coupon']['processed'] / $sync_info['shop_coupon']['count'] * 100 ) : 100,
				$sync_info['shop_order']['processed'],
				$sync_info['shop_order']['count'],
				$sync_info['shop_order']['count'] ? floor( $sync_info['shop_order']['processed'] / $sync_info['shop_order']['count'] * 100 ) : 100
			);
		...
		public function get_sync_details() {
			...
			$where = call_user_func_array( array( $wpdb, 'prepare' ), apply_filters( 'yith_wcmc_get_next_items_to_synch_where', array( '%d=%d', array( 1, 1 ) ) ) );
			...
			$supported_post_type = array( 'product', 'shop_coupon', 'shop_order' );
			$info = array();
		...
		protected function _get_next_items( $post_type, $offset ) {
			...
			switch( $post_type ){
                case 'shop_coupon': $type = 'promo_rule'; break;
                case 'shop_order': $type = 'order'; break;
                default: $type = $post_type;
            }
		...
		public function maybe_process_order( $order ) {
			...
			$args = array_merge(
				...
				( $campaign_processed != 'yes' && isset( $campaign_data['cid'] ) && isset( $campaign_data['eid'] ) ) ? array(
		...
		public function maybe_process_product( $product ) {
			...
			if( $product instanceof WC_Product_Variable ){
				...
				if( ! empty( $variations ) ){
					foreach( $variations as $variation_id ){
						...
						$variants[] = array(
							...
							'price' => $price ? $price : 0,
							'inventory_quantity' => $variation->managing_stock() ? $variation->get_stock_quantity() : 99999999, // use a random high number when not managing stock
				...
				$variants[] = array(
					...
					'sku' => $product->get_sku(),
					'price' => $price ? $price : 0,
                    'inventory_quantity' => $product->managing_stock() ? $product->get_stock_quantity() : 99999999, // use a random high number when not managing stock
				...
				array(
					...
					'customer' => array_merge(
					    array(
                            ...
                            'email_address' => $customer->billing_email,
					...
					'currency_code' => get_woocommerce_currency(),
					'order_total' => array_sum( wp_list_pluck( $saved_cart, 'line_total' ) ),
					'tax_total' => array_sum( wp_list_pluck( $saved_cart, 'line_tax' ) )
				),
				$lines ? array(
					'lines' => $lines
				) : array()
