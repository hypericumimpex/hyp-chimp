...
<?php if( ! empty( $mailchimp_data['options']['choices'] ) ): ?>
	<?php foreach( $mailchimp_data['options']['choices'] as $id => $name ): ?>
		<input type="checkbox" value="<?php echo esc_attr( isset( $mailchimp_data['use_id_instead_of_name'] ) ? $id : $name ) ?>" id="<?php echo esc_attr( $mailchimp_data['tag'] . '_' . $id ) ?>_<?php echo esc_attr( $id ) ?>" name="<?php echo esc_attr( $mailchimp_data['tag'] ) ?>[]"<?php echo ( $mailchimp_data['required'] ) ? 'required="required"' : '' ?>/>
